...
app.post("/login", function (req, res) {
  ...
  pool.getConnection(function (err, connection) {
    connection.query(
      "SELECT * from UTENTI where USERNAME = ? and PASSWORD = ? and UTENTE_ATTIVO = 1",
      [username, password],
      function (err, rows, fields) {
        connection.release();
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              "UPDATE UTENTI SET PASSWORD=?, EDIT_PASSWORD=0 WHERE ID_UTENTE= ?",
              [password, id],
              function (err, rows, fields) {
                if (err) {
                  connection.rollback(function () {
...
// INSERT USER
app.post("/addUser", ensureToken, requireAdminOrResponsabile, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              "INSERT INTO UTENTI (ID_UTENTE, NOME, COGNOME, TIPO, USERNAME, PASSWORD)VALUES (NULL, ?, ?, ?, ?, ?)",
              [nome, cognome, userType, username, password],
              function (err, rows, fields) {
                if (err) {
                  connection.rollback(function () {
                    connection.release();
                  ...
                  if (userType === "OPERATORE" || userType === "BACK_OFFICE") {
                    ...
                    connection.query(
                      "INSERT INTO OPERATORI_VENDITORI (ID_ASSOCIAZIONE, ID_AGENTE, ID_OPERATORE, DATA_INIZIO_ASS, DATA_FINE_ASS)VALUES (NULL, ?, ?, CURDATE(), NULL)",
                      [insertedId, operatoreAssociato],
                      function (err, rows, fields) {
                        if (err) {
                          connection.rollback(function () {
                            connection.release();
                          ...
                          if (supervisoreAssociato) {
                            connection.query(
                              "INSERT INTO SUPERVISORE_RESPONSABILI (ID_ASSOCIAZIONE, ID_SUPERVISORE, ID_RESPONSABILE, DATA_INIZIO_ASS, DATA_FINE_ASS)VALUES (NULL, ?, ?, CURDATE(), NULL)",
                              [supervisoreAssociato, insertedId],
                              function (err, rows, fields) {
                                if (err) {
                                  connection.rollback(function () {
                                    connection.release();
                    ...
                    if (responsabileAssociato) {
                      connection.query(
                        "INSERT INTO RESPONSABILI_AGENTI (ID_ASSOCIAZIONE, ID_RESPONSABILE, ID_AGENTE, DATA_INIZIO_ASS, DATA_FINE_ASS)VALUES (NULL, ?, ?, CURDATE(), NULL)",
                        [responsabileAssociato, insertedId],
                        function (err, rows, fields) {
                          if (err) {
                            connection.rollback(function () {
                              connection.release();
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            if (password == "") {
              queryString =
                "UPDATE UTENTI SET  NOME=?, COGNOME=?, TIPO=?, USERNAME=? WHERE ID_UTENTE=?";
              params = [nome, cognome, userType, username, userId];
            } else {
              queryString =
                "UPDATE UTENTI SET  NOME=?, COGNOME=?, TIPO=?, USERNAME=?, PASSWORD=?, EDIT_PASSWORD=1 WHERE ID_UTENTE=?";
        ...
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            if (oldOperatore != null) {
              connection.query(
                "UPDATE OPERATORI_VENDITORI SET  DATA_FINE_ASS=sysdate()  WHERE ID_AGENTE=? and  DATA_FINE_ASS is null  and ID_OPERATORE=?  ",
                [userId, oldOperatore],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
            ...
            if (newOperatore != null) {
              connection.query(
                "INSERT INTO OPERATORI_VENDITORI (ID_AGENTE, ID_OPERATORE, DATA_INIZIO_ASS, DATA_FINE_ASS) VALUES(?,?,SYSDATE(), null) ",
                [userId, newOperatore],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
        ...
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            if (oldResponsabile != null) {
              // va lasciato lo spazio alla fine di null nella query
              connection.query(
                "UPDATE RESPONSABILI_AGENTI SET  DATA_FINE_ASS=sysdate()  WHERE ID_AGENTE=? and  DATA_FINE_ASS is null and ID_RESPONSABILE=?  ",
                [userId, oldResponsabile],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
                      connection.release();
            ...
            if (newResponsabile != null) {
              connection.query(
                "INSERT INTO RESPONSABILI_AGENTI (ID_AGENTE, ID_RESPONSABILE, DATA_INIZIO_ASS, DATA_FINE_ASS) VALUES(?,?,SYSDATE(), null) ",
                [userId, newResponsabile],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
                      connection.release();
                    ...
                    log.error(
                      "ERRORE SQL INSERT INTO RESPONSABILI_AGENTI : " + err
                    );
                    //errore username duplicato
                    if (err.errno == 1062) {
                      res.sendStatus(400);
        ...
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            if (oldSupervisore != null) {
              // va lasciato lo spazio alla fine di null nella query
              connection.query(
                "UPDATE SUPERVISORE_RESPONSABILI SET  DATA_FINE_ASS=sysdate()  WHERE ID_RESPONSABILE=?  and  DATA_FINE_ASS is null and ID_SUPERVISORE=?  ",
                [userId, oldSupervisore],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
                      connection.release();
                    ...
                    log.error(
                      "ERRORE SQL UPDATE SUPERVISORE_RESPONSABILI: " + err
                    );
                    //errore username duplicato
                    if (err.errno == 1062) {
                      res.sendStatus(400);
            ...
            if (newSupervisore != null) {
              connection.query(
                "INSERT INTO SUPERVISORE_RESPONSABILI (ID_RESPONSABILE, ID_SUPERVISORE, DATA_INIZIO_ASS, DATA_FINE_ASS) VALUES(?,?,SYSDATE(), null) ",
                [userId, newSupervisore],
                function (err, rows, fields) {
                  if (err) {
                    connection.rollback(function () {
                      connection.release();
                    ...
                    log.error(
                      "ERRORE SQL INSERT INTO SUPERVISORE_RESPONSABILI : " + err
                    );
                    //errore username duplicato
                    if (err.errno == 1062) {
                      res.sendStatus(400);
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            var params = [];
            queryString =
              "UPDATE UTENTI SET  UTENTE_ATTIVO=0 WHERE ID_UTENTE=?";
            params = [userId];
...
app.get("/listaOperatoriWS", ensureToken, function (req, res) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          'SELECT * from UTENTI where TIPO = "OPERATORE" AND UTENTE_ATTIVO = 1 ORDER BY COGNOME ASC',
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              data["operatori"] = rows;
...
app.get("/listaAgentiNoRelationWithOperatorWS", ensureToken, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          'SELECT * from UTENTI where (TIPO = "AGENTE" OR TIPO = "RESPONSABILE_AGENTI") AND UTENTE_ATTIVO = 1 ORDER BY COGNOME ASC',
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              data["agenti"] = rows;
...
app.get("/listaAgentiNoRelationWithOperatorWSrecessi", ensureToken, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          'SELECT * from UTENTI where (TIPO = "AGENTE" OR TIPO = "RESPONSABILE_AGENTI" OR TIPO="AGENTE_JUNIOR") AND UTENTE_ATTIVO = 1 ORDER BY COGNOME ASC',
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              data["agenti"] = rows;
...
app.get(
  "/listaAgentiNoRelationWithOperatorAndUserDeletedWS",
  ensureToken,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
      if (err) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            'SELECT * from UTENTI where (TIPO = "AGENTE" OR TIPO = "RESPONSABILE_AGENTI") ORDER BY COGNOME ASC',
            function (err, rows, fields) {
              connection.release();
              if (rows.length !== 0 && !err) {
                data["agenti"] = rows;
...
//lsita Agenti non relazionati agli operatori E ELIMINATI LOGICAMENTE
app.get(
  "/listaAgentiNoRelationWithOperatorAndUserDeletedWSrecessi",
  ensureToken,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
      if (err) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            'SELECT * from UTENTI where (TIPO = "AGENTE" OR TIPO = "RESPONSABILE_AGENTI" OR TIPO = "AGENTE_JUNIOR") ORDER BY COGNOME ASC',
            function (err, rows, fields) {
              connection.release();
              if (rows.length !== 0 && !err) {
                data["agenti"] = rows;
...
app.get("/listaUtenti", ensureToken, requireAdmin, function (req, res) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT * from UTENTI WHERE UTENTE_ATTIVO=1 ORDER BY COGNOME ASC",
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              data["utenti"] = rows;
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              "UPDATE APPUNTAMENTI SET DATA_APPUNTAMENTO = ?, ORA_APPUNTAMENTO = ?, " +
              ...
              [
                ...
                numLuce,
                numGas,
                codici_contratto_luce,
                codici_contratto_gas,
                date_ok,
                idAppuntamento,
              ],
              function (err, rows, fields) {
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              "UPDATE APPUNTAMENTI SET ESITO = ?, NUM_LUCE = ?, NUM_GAS = ?, CODICI_CONTRATTO_LUCE = ?, CODICI_CONTRATTO_GAS = ?, NOTE_AGENTE = ?, DATA_OK = ? WHERE ID_APPUNTAMENTO = ?",
              [
                esitoAppuntamento,
                numLuce,
                numGas,
...
app.get(
  "/listaAppuntamentiAdmin",
  ensureToken,
  requireAdminOrBackOffice,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
      if (err) {
        res.sendStatus(403);
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            "SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE," +
              ...
              ' OR ESITO = "VALUTA" OR ESITO = "ASSENTE" OR ESITO = "NON VISITATO" OR (ESITO IS NULL AND DATA_APPUNTAMENTO <= ?)',
            [from, to, to],
            function (err, rows, fields) {
              connection.release();
              if (err) {
                ...
                if (rows.length !== 0) {
                  ...
                  data["appuntamenti"] =
                    "Errore in fase di reperimento appuntamentI";
                  res.status(500).json(data);
                  console.log(
                    "Errore in fase di reperimento appuntamenti: " + err
                  );
                  log.error(
                    "Errore in fase di reperimento appuntamenti: " + err
                  );
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE," +
            ...
            " LEFT JOIN UTENTI VENDITORE ON APPUNTAMENTI.ID_VENDITORE=VENDITORE.ID_UTENTE" +
            " WHERE ((DATA_APPUNTAMENTO >= ? AND DATA_APPUNTAMENTO <= ?)" +
            ' OR (ESITO = "VALUTA" OR ESITO = "ASSENTE" OR ESITO = "NON VISITATO" OR (ESITO IS NULL AND DATA_APPUNTAMENTO <= ?))) AND APPUNTAMENTI.ID_VENDITORE=?',
          [from, to, to, id],
          function (err, rows, fields) {
            connection.release();
            if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE," +
            ...
            " LEFT JOIN UTENTI VENDITORE ON APPUNTAMENTI.ID_VENDITORE=VENDITORE.ID_UTENTE" +
            " WHERE ((DATA_APPUNTAMENTO >= ? AND DATA_APPUNTAMENTO <= ?)" +
            ' OR (ESITO = "VALUTA" OR ESITO = "ASSENTE" OR ESITO = "NON VISITATO" OR (ESITO IS NULL AND DATA_APPUNTAMENTO <= ?))) AND APPUNTAMENTI.ID_OPERATORE = ?',
          [from, to, to, id],
          function (err, rows, fields) {
            connection.release();
            if (err) {
        ...
        connection.query(
          ` SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE,  
			...
			SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE,  
				   ...
				   (ESITO = "VALUTA" OR ESITO = "ASSENTE" OR ESITO = "NON VISITATO"  
				   OR (ESITO IS NULL AND DATA_APPUNTAMENTO <= ?))) `,
          [id, from, to, to, id, from, to, to, id, from, to, to],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              log.error("ERRORE SQL LISTA APPUNTAMENTI " + err);
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            log.error(
              "ERRORE SQL TRANSACTION CANCELLAZIONE APPUNTAMENTO " + errTrans
            );
            res.sendStatus(500);
          } else {
            connection.query(
...
//Appuntamento
app.get(
  "/listaNomiAziendaAndIdAppunamentoForProvincia/:provincia",
  ensureToken,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
      if (err) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            `SELECT APPUNTAMENTI.ID_APPUNTAMENTO, APPUNTAMENTI.NOME_ATTIVITA FROM APPUNTAMENTI WHERE PROVINCIA = ? `,
            [provincia],
            function (err, rows, fields) {
              connection.release();
              if (err) {
                log.error(
                  "ERRORE SQL GET LISTA NOME AZIENDA: " +
                    provincia +
                    " --> " +
                    err
                );
                ...
                if (rows.length !== 0) {
                  ...
                  //Error code 2 = no rows in db.
                  data["error"] = 2;
                  data["listaAziendaAndIdAppuntamento"] =
                    "Nessuna lista nomi azienda trovata";
                  res.status(404).json(data);
                } else {
                  data["appuntamento"] =
                    "Errore in fase di reperimento lista nomi azienda";
                  res.status(500).json(data);
                  console.log(
                    "Errore in fase di reperimento lista nomi azienda: " + err
                  );
                  log.error(
                    "Errore in fase di reperimento lista nomi azienda: " + err
                  );
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE," +
            ...
            " LEFT JOIN UTENTI OPERATORE ON APPUNTAMENTI.ID_OPERATORE=OPERATORE.ID_UTENTE" +
            " LEFT JOIN UTENTI VENDITORE ON APPUNTAMENTI.ID_VENDITORE=VENDITORE.ID_UTENTE" +
            " WHERE APPUNTAMENTI.ID_APPUNTAMENTO = ?",
          [idAppuntamento],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              log.error(
                "ERRORE SQL GET APPUNTAMENTO: " + idAppuntamento + " --> " + err
              );
              res.sendStatus(500);
            } else {
              if (rows.length !== 0) {
                ...
                data["appuntamento"] =
                  "Errore in fase di reperimento appuntamento";
                res.status(500).json(data);
                console.log(
                  "Errore in fase di reperimento appuntamento: " + err
                );
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          `  SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE, VENDITORE.NOME NOME_VENDITORE, VENDITORE.COGNOME COGNOME_VENDITORE, a2.* 
                ...
                and a2.ID_APPUNTAMENTO != ? ORDER BY DATA_APPUNTAMENTO DESC `,
          [idAppuntamento, idAppuntamento],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              log.error(
                "ERRORE SQL GET appuntamentiCorrelati: " +
                  idAppuntamento +
                  " --> " +
                  err
              );
  ...
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      if (
        ...
        QragioneSociale =
          ' AND LOWER(NOME_ATTIVITA) LIKE LOWER("%' + ragioneSociale + '%") ';
      }
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT COUNT(*) AS TotalCount from APPUNTAMENTI WHERE 1=1 " +
            QdateFrom +
            QdateTo +
            Qprovincia +
            Qcomune +
              ...
              pool.getConnection(function (err, connection) {
                connection.query(
                  "SELECT OPERATORE.NOME NOME_OPERATORE, OPERATORE.COGNOME COGNOME_OPERATORE," +
                    ...
                    Qoperatore +
                    Qesito +
                    " ORDER BY DATA_APPUNTAMENTO DESC LIMIT ? OFFSET ?",
                  [limit, offset],
                  function (err, rows, fields) {
                    connection.release();
                    if (err) {
...
app.post(
  "/downloadAppuntamenti",
  ensureToken,
  requireAdminOrResponsabile,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
      if (err) {
        res.sendStatus(403);
        ...
        if (
          codiceLuce !== "" &&
          codiceLuce !== undefined &&
          codiceLuce != null
        ) {
          QcodiceLuce =
            ' AND CODICI_CONTRATTO_LUCE LIKE "%' + codiceLuce + '%" ';
        }
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            'SELECT CONCAT(VENDITORE.COGNOME, " ",  VENDITORE.NOME) AS "Agente", ' +
              ...
              Qoperatore +
              Qesito +
              " ORDER BY DATA_APPUNTAMENTO DESC ",
            [],
            function (err, rows, fields) {
              connection.release();
              if (err) {
...
app.post("/searchDateResponsabile", ensureToken, function (req, res) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      if (dateFrom !== "" && dateFrom !== undefined && dateFrom != null) {
        QdateFrom =
          ' AND APPUNTAMENTI.DATA_APPUNTAMENTO >= "' + dateFrom + '" ';
      }
      ...
      if (
        ...
        QragioneSociale =
          ' AND LOWER(APPUNTAMENTI.NOME_ATTIVITA) LIKE LOWER("%' +
          ragioneSociale +
          '%") ';
      }
      ...
      if (codiceLuce !== "" && codiceLuce !== undefined && codiceLuce != null) {
        QcodiceLuce =
          ' AND APPUNTAMENTI.CODICI_CONTRATTO_LUCE LIKE "%' +
          codiceLuce +
          '%" ';
      }
      ...
      if (codiceGas !== "" && codiceGas !== undefined && codiceGas != null) {
        QcodiceGas =
          ' AND APPUNTAMENTI.CODICI_CONTRATTO_GAS LIKE "%' + codiceGas + '%" ';
      }
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT COUNT(*) AS TotalCount  FROM( SELECT  APPUNTAMENTI.ID_APPUNTAMENTO  " +
            ...
            " " +
            " ) SOMMA_APPUNTAMENTI",
          [idResponsabile, idResponsabile, idResponsabile, idResponsabile],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              log.error("ERRORE SQL RICERCA COUNT APPUNTAMENTI " + err);
  ...
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      if (
        ...
        QragioneSociale =
          ' AND LOWER(APP.NOME_ATTIVITA) LIKE LOWER("%' +
          ragioneSociale +
          '%") ';
      }
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          ...
          function (err, rows, fields) {
            ...
            if (err) {
              log.error(
                "ERRORE SQL RICERCA COUNT APPUNTAMENTI_ricontatto " + err
              );
              res.sendStatus(500);
            } else {
              data["totaleAppuntamenti"] = rows[0].TotalCount;
              ...
              pool.getConnection(function (err, connection) {
                connection.query(
                  ...
                  function (err, rows, fields) {
                    ...
                    if (err) {
                      log.error(
                        "ERRORE SQL RICERCA appuntamentiDaRicontattare: --> " +
                          err
                      );
                      res.sendStatus(500);
                    } else {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          ...
          function (err, rows, fields) {
            ...
            if (err) {
              log.error(
                "ERRORE SQL GET STORICO RICONTATTI PER APPUNTAMENTO: " +
                  idAppuntamento +
                  " --> " +
                  err
              );
              ...
              if (rows.length !== 0) {
                ...
                data["storicoRicontatti"] =
                  "Errore in fase di reperimento STORICO RICONTATTI appuntamento";
                res.status(500).json(data);
                console.log(
                  "Errore in fase di reperimento STORICO RICONTATTI appuntamento: " +
                    err
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              `UPDATE APPUNTAMENTI SET NON_RICONTATTARE=1 WHERE ID_APPUNTAMENTO= ? `,
              [idAppuntamento],
              function (err, rows, fields) {
                if (err) {
                  connection.rollback(function () {
                  ...
                  log.error(
                    "ERRORE SQL UPDATE APP_DA_RICONTATTATTARE   " + err
                  );
                  res.sendStatus(500);
                } else {
                  connection.commit(function (err) {
...
app.post("/verifyDate", ensureToken, requireAdminOrBackOffice, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            `select
                  ...
                  WHERE
                    dettaglio_recesso_gas.DATA_INSERIMENTO <= ? `,
            [dateToTodayMenoUno, dateToTodayMenoUno],
            function (err, rows, fields) {
              connection.release();
              if (err) {
                ...
                if (rows.length !== 0 && !err) {
                  ...
                  data["stats"] =
                    "Errore in fase di reperimento statistiche dashboard";
                  res.status(500).json(data);
                  console.log(
                    "Errore in fase di reperimento statistiche dashboard: " +
                      err
...
app.get("/recessesStatsVenditoreDashboard/:id", ensureToken, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
        ...
        connection.query(
          `select
                  ...
                  from
                    dettaglio_recesso_gas
                  WHERE
                    dettaglio_recesso_gas.DATA_INSERIMENTO <= ? AND dettaglio_recesso_gas.VENDITORE_ASSEGNATO = ?`,
          [dateToTodayMenoUno, idVenditore, dateToTodayMenoUno, idVenditore],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              log.error("ERRORE SQL STATS DASHBOARD ADMIN: --> " + err);
              ...
              if (rows.length !== 0 && !err) {
                ...
                res.status(500).json(data);
                console.log(
                  "Errore in fase di reperimento statistiche dashboard: " + err
                );
                log.error(
                  "Errore in fase di reperimento statistiche dashboard: " + err
                );
          ...
          function (err, rows, fields) {
            ...
            if (err) {
              ...
              if (rows.length !== 0 && !err) {
                ...
                data["stats"] =
                  "Errore in fase di reperimento statistiche dashboard venditore";
                res.status(500).json(data);
                console.log(
                  "Errore in fase di reperimento statistiche dashboard venditore: " +
                    err
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          "SELECT * FROM UTENTI AS UT " +
            " JOIN " +
            " (SELECT * FROM OPERATORI_VENDITORI AS OP WHERE OP.ID_OPERATORE = ? AND OP.DATA_FINE_ASS IS NULL ) AS T ON UT.ID_UTENTE=T.ID_AGENTE WHERE UT.UTENTE_ATTIVO = 1",
          [idOperatore],
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
        ...
        connection.query(
          `SELECT UT.* FROM UTENTI AS UT
				  ...
				  UNION
					  ...
					  ON UT.ID_UTENTE=T.ID_AGENTE WHERE UT.UTENTE_ATTIVO = 1`,
          [idResponsabile, idResponsabile, idResponsabile],
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              data["utenti"] = rows;
...
app.get("/listaResponsabiliAgentiWS", ensureToken, function (req, res) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          'SELECT * from UTENTI where TIPO = "RESPONSABILE_AGENTI" AND UTENTE_ATTIVO = 1 ORDER BY COGNOME ASC',
          function (err, rows, fields) {
            connection.release();
            if (rows.length !== 0 && !err) {
              ...
              data["responsabili"] =
                "Errore in fase di reperimento responsabili";
              res.status(500).json(data);
            ...
            connection.beginTransaction(function (errTrans) {
              if (errTrans) {
                ...
                jsonLuce.forEach((recesso) => {
                  ...
                  recesso.DATA_VALIDITA_RECESSO = new Date(
...
app.post("/insertRecessesGas", ensureToken, requireAdminOrBackOffice, function (
  req,
  res
) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
          ...
          connection.beginTransaction(function (errTrans) {
            if (errTrans) {
              ...
              jsonGas.forEach((recesso) => {
                recesso.DATA_INIZIO_FORNITURA = new Date(
...
app.get(
  "/listaAgenzieNonAssociatiRecessiGas",
  ensureToken,
  requireAdminOrResponsabile,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            ...
            function (err, rows, fields) {
              ...
              if (err) {
                log.error(
                  "ERRORE SQL RICERCA listaAgenzieNonAssociatiRecessiGas: --> " +
                    err
                );
                res.sendStatus(500);
              } else {
...
app.get(
  "/listaAgenzieNonAssociatiRecessiLuce",
  ensureToken,
  requireAdminOrResponsabile,
  function (req, res) {
    jwt.verify(req.token, config.secretKey, function (err, data) {
        ...
        pool.getConnection(function (err, connection) {
          connection.query(
            ...
            function (err, rows, fields) {
              ...
              if (err) {
                log.error(
                  "ERRORE SQL RICERCA listaAgenzieNonAssociatiRecessiLuce: --> " +
                    err
                );
                res.sendStatus(500);
              } else {
  ...
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      if (
        ...
        QragioneSociale =
          ' AND LOWER(DENOMINAZIONE) LIKE LOWER("%' + ragioneSociale + '%") ';
      }
      ...
      if (stato !== "" && stato !== undefined && stato != null) {
        ...
        if (
          req.body.userType &&
          (req.body.userType == "AGENTE" ||
            req.body.userType == "AGENTE_JUNIOR" ||
            req.body.userType == "RESPONSABILE_AGENTI")
        ) {
          //DI DEFAULT NON MOSTRO I NON GESTIRE  I RESPINTI e I RIENTRI
          Qstato =
            ' AND STATO != "NON_GESTIRE" AND STATO != "RESPINTO" AND STATO != "RIENTRO" ';
        } else {
          //DI DEFAULT NON MOSTRO I NON GESTIRE  I RESPINTI e I RIENTRI
      ...
      if (mcAnnui !== "" && mcAnnui !== undefined && mcAnnui != null) {
        if (mcAnnui == "50000") {
          ...
          QmcAnnui =
            " AND CONSUMO_CONTRATTUALE BETWEEN " + from + " AND " + to + " ";
        }
      }
      ...
      if (
        agenziaOriginaria !== "" &&
        agenziaOriginaria !== undefined &&
        agenziaOriginaria != null
      ) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          `SELECT COUNT(*) AS TotalCount from recessi_gas as rg inner join dettaglio_recesso_gas as drg on rg.ID_RECESSO_GAS = drg.ID_RECESSO_GAS
              ...
              pool.getConnection(function (err, connection) {
                connection.query(
                  `select 
                                ...
                                where 1=1 ${QdateFrom}  ${QdateTo}  ${Qprovincia} ${Qcomune} ${QragioneSociale}  ${Qagente}  ${QagenziaOriginaria}  ${Qstato}  ${QmcAnnui}  ORDER BY ${Qorder} DATA_OUT DESC LIMIT ? OFFSET ?`,
                  [limit, offset],
                  function (err, rows, fields) {
                    connection.release();
                    if (err) {
  ...
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      if (
        ...
        QragioneSociale =
          ' AND LOWER(RAGIONE_SOCIALE) LIKE LOWER("%' + ragioneSociale + '%") ';
      }
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          `SELECT COUNT(*) AS TotalCount from recessi_luce as rl inner join dettaglio_recesso_luce as drl on rl.ID_RECESSO_LUCE = drl.ID_RECESSO_LUCE
              ...
              pool.getConnection(function (err, connection) {
                connection.query(
                  `select 
                                ...
                                where 1=1 ${QdateFrom}  ${QdateTo}  ${Qprovincia} ${Qcomune} ${QragioneSociale} ${QagenziaOriginaria} ${Qagente}  ${Qstato} ${QkwhAnnui} ORDER BY  ${Qorder} DATA_VALIDITA_RECESSO DESC LIMIT ? OFFSET ?`,
                  [limit, offset],
                  function (err, rows, fields) {
                    connection.release();
                    if (err) {
...
app.get("/checkRecessiAgente/:id", ensureToken, function (req, res) {
  jwt.verify(req.token, config.secretKey, function (err, data) {
    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.query(
          `SELECT rl.ID_RECESSO_LUCE  from recessi_luce as rl inner join dettaglio_recesso_luce as drl on rl.ID_RECESSO_LUCE = drl.ID_RECESSO_LUCE where drl.VENDITORE_ASSEGNATO=? and drl.STATO = 'ASSEGNATO' `,
          [agente],
          function (err, rows, fields) {
            connection.release();
            if (err) {
              ...
              pool.getConnection(function (err, connection) {
                connection.query(
                  `SELECT rg.ID_RECESSO_GAS from recessi_gas as rg inner join dettaglio_recesso_gas as drg on rg.ID_RECESSO_GAS = drg.ID_RECESSO_GAS where drg.VENDITORE_ASSEGNATO=? and drg.STATO = 'ASSEGNATO'`,
                  [agente],
                  function (err, rows, fields) {
                    connection.release();
                    if (err) {
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              `UPDATE dettaglio_recesso_luce SET VENDITORE_ASSEGNATO = ?, REFERENTE_RECESSO=?, REFERENTE_RECESSO_RECAPITO=?, STATO=?, COD_CONTRATTO=?, ULTIMA_MODIFICA=NOW(), NOTE=? WHERE ID_DETTAGLIO_LUCE=?`,
              [
                agente,
                refRecesso,
                refRecapito,
      ...
      pool.getConnection(function (err, connection) {
        connection.beginTransaction(function (errTrans) {
          if (errTrans) {
            ...
            connection.query(
              `UPDATE dettaglio_recesso_gas  SET VENDITORE_ASSEGNATO = ?, REFERENTE_RECESSO=?, REFERENTE_RECESSO_RECAPITO=?, STATO=?, COD_CONTRATTO=?, ULTIMA_MODIFICA=NOW(), NOTE=? WHERE ID_DETTAGLIO_GAS=?`,
              [
                agente,
                refRecesso,
                refRecapito,
