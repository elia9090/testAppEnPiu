 ...
 */
    ...
    $scope.recessesLuce.downloadRecessiLuce = function () {

        if ($scope.recessesLuce.totalItems && $scope.recessesLuce.totalItems > 5000) {
            alertify.alert("Ci sono più di 5000 recessi Luce! Restingere la ricerca per l'export in Excel");
            return;
        }
        ...
        $http.post('/downloadRecessiLuce', {
            ...
            if (result.data.recessiLuce.length == 0) {
                ...
                var fileName = 'EstrazioneRecessiLuce_del_' + today + '.xlsx';

                var ws = XLSX.utils.json_to_sheet(result.data.recessiLuce);
                var wb = XLSX.utils.book_new();
                XLSX.utils.book_append_sheet(wb, ws, 'Recessi Luce');
            ...
            if (err.status === 500) {
                alertify.alert("Errore nel download dei recessi Luce");
            }
