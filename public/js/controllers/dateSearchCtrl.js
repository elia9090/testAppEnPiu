app.controller('dateSearchCtrl', ['$scope', '$http', '$location', 'alertify', function ($scope, $http, $location, alertify) {
    ...
    $scope.searchDate.downloadAppuntamenti = function () {

        if ($scope.searchDate.totalItems && $scope.searchDate.totalItems > 5000) {
            alertify.alert("Ci sono più di 5000 appuntamenti! Restingere la ricerca per l'export in Excel");
            return;
        }

        $http.post('/downloadAppuntamenti', {
            'dateFROM': $scope.searchDate.searchParam.dataAppuntamentoDAL,
            'dateTO': $scope.searchDate.searchParam.dataAppuntamentoAL,
            'provincia': $scope.searchDate.searchParam.provinciaSelected,
            'comune': $scope.searchDate.searchParam.comuneSelected,
            ...
            if (result.data.appuntamenti.length == 0) {
                ...
                var fileName = 'EstrazioneAppuntamenti_del_' + today + '.xlsx';

                var ws = XLSX.utils.json_to_sheet(result.data.appuntamenti);
            ...
            if (err.status === 500) {
                alertify.alert("Errore nel download degli appuntamenti");
            }
